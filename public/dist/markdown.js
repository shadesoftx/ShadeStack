...
// resources/js/markdown/display.ts
var Display = class {
  constructor(editor) {
    __publicField(this, "editor");
    __publicField(this, "container");
    __publicField(this, "doc", null);
    __publicField(this, "lastDisplayClick", 0);
    this.editor = editor;
    ...
    editor.settings.onChange("showPreview", (show2) => this.updateVisibility(Boolean(show2)));
  }
  updateVisibility(show2) {
    const wrap = this.container.closest(".markdown-editor-wrap");
    wrap.style.display = show2 ? "" : "none";
  }
  onLoad() {
...
var _Actions_instances, insertDrawing_fn, replaceSelection_fn, cleanTextForEditor_fn, findAndReplaceContent_fn, wrapLine_fn;
var Actions = class {
  constructor(editor) {
...
`, { from: selectFrom });
  ...
  replaceLineStart(newStart) {
    ...
    if (lineStart === newStart) {
      ...
      this.editor.input.spliceText(lineRange.from, lineRange.to, newLineContent2, { from: selectFrom2 });
      return;
    }
  ...
  cycleCalloutTypeAtSelection() {
    ...
    const format2 = (matches ? matches[2] || matches[3] : "").toLowerCase();
    if (format2 === formats[formats.length - 1]) {
      __privateMethod(this, _Actions_instances, wrapLine_fn).call(this, `<p class="callout ${formats[formats.length - 1]}">`, "</p>");
    } else if (format2 === "") {
      __privateMethod(this, _Actions_instances, wrapLine_fn).call(this, '<p class="callout info">', "</p>");
    } else if (matches) {
      const newFormatIndex = formats.indexOf(format2) + 1;
      const newFormat = formats[newFormatIndex];
      const newContent = lineText.replace(matches[0], matches[0].replace(format2, newFormat));
   ...
   */
  async insertTemplate(templateId, event) {
    const cursorPos = this.editor.input.eventToPosition(event).from;
    const responseData = (await window.$http.get(`/templates/${templateId}`)).data;
   ...
   */
  insertClipboardImages(images, event) {
    const cursorPos = this.editor.input.eventToPosition(event).from;
  ...
  async uploadImage(file, position = null) {
    ...
    try {
      const image2 = (await window.$http.post("/images/gallery", formData)).data;
      const newContent = `[![](${image2.thumbs.display})](${image2.url})`;
 ...
 */
replaceSelection_fn = function(newContent, offset = 0, selection = null) {
...
findAndReplaceContent_fn = function(search, replace2) {
  const newText = this.editor.input.getText().replace(search, replace2);
  ...
  loadFromLocalStorage() {
    ...
    try {
      ...
      for (const [key, value] of Object.entries(lsVals)) {
        if (value !== null && value !== void 0 && key in this.settingMap) {
          this.settingMap[key] = value;
        }
      }
    } catch (error2) {
      console.warn("Failed to parse settings from localStorage:", error2);
    }
  }
  set(key, value) {
    this.settingMap[key] = value;
...
function listenToCommonEvents(editor) {
...
function shortcutsToKeyBindings(shortcuts) {
  const keyBindings = [];
...
async function init2(input, shortcuts, domEventHandlers, onChange) {
...
var CodemirrorInput = class {
  ...
  setSelection(selection, scrollIntoView = false) {
    this.cm.dispatch({
  ...
  setText(text2, selection) {
    selection = selection || this.getSelection();
    const newDoc = this.cm.state.toText(text2);
    const newSelectFrom = Math.min(selection.from, newDoc.length);
    const scrollTop = this.cm.scrollDOM.scrollTop;
    this.dispatchChange(0, this.cm.state.doc.length, text2, newSelectFrom);
    this.focus();
    window.requestAnimationFrame(() => {
      this.cm.scrollDOM.scrollTop = scrollTop;
    });
  }
  spliceText(from, to, newText, selection = null) {
...
${text2}`);
  ...
  getLineRangeFromPosition(position) {
    const line = this.cm.state.doc.lineAt(position);
    return { from: line.from, to: line.to };
  }
  searchForLineContaining(text2) {
    const docText = this.cm.state.doc;
    let lineCount = 1;
    let scrollToLine = -1;
    for (const line2 of docText.iterLines()) {
      if (line2.includes(text2)) {
   ...
   */
  dispatchChange(from, to = null, text2 = null, selectFrom = null, selectTo = null) {
    const change = { from };
    if (to) {
      change.to = to;
    }
...
var UndoStack = class {
  constructor() {
    __publicField(this, "onChangeDebounced");
    __publicField(this, "stack", []);
    __publicField(this, "pointer", -1);
    __publicField(this, "lastActionTime", 0);
    this.onChangeDebounced = debounce(this.onChange, 1e3, false);
  }
  undo() {
    if (this.pointer < 1) {
      return null;
...
var TextareaInput = class {
  constructor(input, shortcuts, events, onChange) {
    __publicField(this, "input");
    __publicField(this, "shortcuts");
    __publicField(this, "events");
    __publicField(this, "onChange");
    __publicField(this, "eventController", new AbortController());
    __publicField(this, "undoStack", new UndoStack());
    __publicField(this, "textSizeCache", null);
    this.input = input;
...
${text2}`;
  ...
  getSelectionText(selection) {
    ...
    return text2.slice(range.from, range.to);
  }
  getText() {
  ...
  searchForLineContaining(text2) {
    const textPosition = this.getText().indexOf(text2);
    if (textPosition > -1) {
  ...
  spliceText(from, to, newText, selection) {
    const text2 = this.getText();
    const updatedText = text2.slice(0, from) + newText + text2.slice(to);
...
b`;
  ...
  measureLineCharCount(textWidth) {
    const inputStyles = window.getComputedStyle(this.input);
  ...
  mouseEventToTextRelativeCoords(event) {
...
function provideShortcutMap(editor) {
  const shortcuts = {};
...
function getMarkdownDomEventHandlers(editor) {
...
async function init3(config2) {
  ...
  const initCodemirrorInput = async () => {
    const codeMirror = await init2(config2.inputEl, shortcuts, eventHandlers, onInputChange);
    return new CodemirrorInput(codeMirror);
  };
  const initTextAreaInput = async () => {
    return new TextareaInput(config2.inputEl, shortcuts, eventHandlers, onInputChange);
  };
