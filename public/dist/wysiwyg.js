...
var KEY_AT_COMMAND = createCommand("KEY_AT_COMMAND");
...
var DetailsNode2 = class _DetailsNode extends ElementNode8 {
  constructor() {
    super(...arguments);
    __publicField(this, "__id", "");
...
function $createDetailsNode() {
  return new DetailsNode2();
}
function $isDetailsNode(node) {
  return node instanceof DetailsNode2;
}
...
var MentionNode = class _MentionNode extends DecoratorNode3 {
  constructor() {
    super(...arguments);
    __publicField(this, "__user_id", 0);
    __publicField(this, "__user_name", "");
    __publicField(this, "__user_slug", "");
  }
  static getType() {
    return "mention";
  }
  static clone(node) {
    const newNode = new _MentionNode(node.__key);
    newNode.__user_id = node.__user_id;
    newNode.__user_name = node.__user_name;
    newNode.__user_slug = node.__user_slug;
    return newNode;
  }
  setUserDetails(userId, userName, userSlug) {
    const self = this.getWritable();
    ...
    return {
      a(node) {
        if (node.hasAttribute("data-mention-user-id")) {
          return {
            conversion: (element) => {
              const node2 = new _MentionNode();
              node2.setUserDetails(
...
function $createMentionNode(userId, userName, userSlug) {
...
function getNodesForCommentEditor() {
  return [
...
var EditorFormModal = class extends EditorContainerUiElement {
  constructor(definition, key) {
    ...
    __publicField(this, "definition");
    __publicField(this, "key");
    __publicField(this, "originalFocus", null);
    this.definition = definition;
    this.key = key;
  }
  show(defaultValues) {
...
var EditorOverflowContainer = class extends EditorContainerUiElement {
  constructor(label, size, children) {
    super(children);
...
// resources/js/wysiwyg/ui/decorators/CodeBlockDecorator.ts
  ...
  setup(element) {
    ...
    element.addEventListener("dblclick", (event) => {
      this.context.editor.getEditorState().read(() => {
        $openCodeEditorForNode(this.context.editor, this.getNode());
      });
    });
...
// resources/js/wysiwyg/ui/decorators/DiagramDecorator.ts
var DiagramDecorator = class extends EditorDecorator {
  ...
  setup(element) {
    ...
    this.context.editor.registerCommand(CLICK_COMMAND, (event) => {
    ...
    element.addEventListener("dblclick", (event) => {
      this.context.editor.getEditorState().read(() => {
        $openDrawingEditorForNode(this.context, this.getNode());
      });
    });
    const selectionChange = (selection) => {
...
var _button, _isActive;
var EditorApiButton = class {
  constructor(options, context) {
    __privateAdd(this, _button);
    __privateAdd(this, _isActive, false);
    __privateSet(this, _button, new EditorButton({
      label: options.label || "",
      icon: options.icon || "",
      action: () => {
        options.action();
      },
      isActive: () => __privateGet(this, _isActive)
    }));
    __privateGet(this, _button).setContext(context);
  }
  setActive(active = true) {
    __privateSet(this, _isActive, active);
    __privateGet(this, _button).setActiveState(active);
  }
  _getOriginalModel() {
...
var _toolbar;
var EditorApiToolbar = class {
  constructor(toolbar) {
    __privateAdd(this, _toolbar);
    __privateSet(this, _toolbar, toolbar);
  }
  getSections() {
    const sections = __privateGet(this, _toolbar).getChildren();
...
var _section;
var EditorApiToolbarSection = class {
  constructor(section) {
    __privateAdd(this, _section);
    __privateSet(this, _section, section);
  }
  getLabel() {
    return __privateGet(this, _section).getLabel();
  }
  addButton(button, targetIndex = -1) {
    __privateGet(this, _section).addChild(button._getOriginalModel(), targetIndex);
    __privateGet(this, _section).rebuildDOM();
  }
};
_section = new WeakMap();
var _context;
var EditorApiUiModule = class {
  constructor(context) {
    __privateAdd(this, _context);
    __privateSet(this, _context, context);
  }
  createButton(options) {
    return new EditorApiButton(options, __privateGet(this, _context));
  }
  getMainToolbar() {
    const toolbar = __privateGet(this, _context).manager.getToolbar();
    if (!toolbar) {
      return null;
    }
...
var _context2;
var EditorApiContentModule = class {
  constructor(context) {
    __privateAdd(this, _context2);
    __privateSet(this, _context2, context);
  }
  insertHtml(html, position = "selection") {
    ...
    if (position === "start") {
      prependHtmlToEditor(__privateGet(this, _context2).editor, html);
    } else if (position === "end") {
      appendHtmlToEditor(__privateGet(this, _context2).editor, html);
    } else {
      insertHtmlIntoEditor(__privateGet(this, _context2).editor, html);
    }
  }
};
...
function isHTMLElement2(el3) {
  return el3 instanceof HTMLElement;
}
...
var KeyboardNavigationHandler = class {
  ...
  focusNext() {
    ...
    const currentIndex = isHTMLElement2(activeEl) ? focusable.indexOf(activeEl) : -1;
...
keydownHandler_fn = function(event) {
  if (isHTMLElement2(event.target) && event.target.matches("input") && (event.key === "ArrowRight" || event.key === "ArrowLeft")) {
    return;
  }
  if (event.key === "ArrowDown" || event.key === "ArrowRight") {
    ...
    if (this.onEscape) {
      this.onEscape(event);
    } else if (isHTMLElement2(document.activeElement)) {
...
getFocusable_fn = function() {
  ...
  for (const container of this.containers) {
    const toAdd = [...container.querySelectorAll(selector)].filter((e) => isHTMLElement2(e));
...
function userClickHandler(onSelect) {
  return (event) => {
    const userItem = event.target.closest("a[data-id]");
    if (!userItem) {
      return;
    }
...
function handleUserSelectCancel(context, selectList, controller, onCancel) {
  selectList.addEventListener("keydown", (event) => {
    if (event.key === "Escape") {
      onCancel();
    }
  }, { signal: controller.signal });
  const input = selectList.querySelector("input");
...
function handleUserListLoading(selectList) {
  const cache = /* @__PURE__ */ new Map();
  const updateUserList = async (searchTerm) => {
    ...
    for (const listEl of toInsert) {
      const adopted = window.document.adoptNode(listEl);
      selectList.appendChild(adopted);
    }
  };
  updateUserList("");
  const input = selectList.parentElement?.querySelector("input");
...
function buildAndShowUserSelectorAtElement(context, mentionDOM) {
  const searchInput = el("input", { type: "text" });
  const list = el("div", { class: "dropdown-search-list" });
  const iconWrap = el("div");
  iconWrap.innerHTML = search_default;
...
var MentionDecorator = class extends EditorDecorator {
  constructor() {
    super(...arguments);
    __publicField(this, "abortController", null);
    __publicField(this, "dropdownContainer", null);
    __publicField(this, "mentionElement", null);
  }
  setup(element) {
  ...
  showSelection() {
    ...
    handleUserListLoading(this.dropdownContainer.querySelector(".dropdown-search-list"));
    this.dropdownContainer.addEventListener("click", userClickHandler((id, name, slug) => {
      this.context.editor.update(() => {
        const mentionNode = this.getNode();
        this.hideSelection();
...
function enterUserSelectMode(context, selection) {
...
function selectMention(context, event) {
  const selected = $getSelection()?.getNodes() || [];
  if (selected.length === 1 && $isMentionNode(selected[0])) {
    const mention = selected[0];
    const decorator = context.manager.getDecoratorByNodeKey(mention.getKey());
...
function registerMentions(context) {
  const editor = context.editor;
  const unregisterCommand = editor.registerCommand(KEY_AT_COMMAND, function(event) {
    const selection = $getSelection();
    if ($isRangeSelection(selection) && selection.isCollapsed()) {
      window.setTimeout(() => {
        editor.update(() => {
  ...
  const unregisterEnter = editor.registerCommand(KEY_ENTER_COMMAND, function(event) {
...
function createCommentEditorInstance(container, htmlContent, options = {}) {
  const editor = createEditor({
